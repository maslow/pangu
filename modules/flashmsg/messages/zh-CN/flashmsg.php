<?php
/**
 * Created by PhpStorm.
 * User: wangfugen
 * Date: 15/11/2
 * Time: 下午11:05
 */

return [
    'Create role successfully!' => '创建角色成功！',

    'You have no permission with the operation : {permission}' => '您没有进行该操作的权限：{permission}',
    //'Failed to login, please try again.' => '登录失败，请重试！',
    //'Please login to continue your operation or your session is timeout.' => '请登录后再进行操作或者您的登录已过期!',
    //'You have been logged-out, bye!' => '您已安全退出，再见!',
    'Success to create manager!' => '创建管理员成功！',
    'Failed to create manager!' => '创建管理员失败！',
    'Success to update manager!' => '更新管理员成功！',
    'Failed to update manager!' => '更新管理员失败！',
    'Success to delete manager!' => '删除管理员成功！',
    'Failed to delete manager!' => '删除管理员失败！',
];